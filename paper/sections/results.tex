...
Paradigm Free Mapping bases its selection of the regularization parameter on the BIC. Hence, we calculated the regularization path with PFM by means of the least angle regression (LARS) algorithm (\citealt{efron2004LeastAngleRegression}) and used the \(\lambda\)s from the regularization path\todo{To be explained above in Algorithms.} to solve the deconvolution problem with Total Activation.
...
\todo[inline]{What about computation time?}
...
\begin{figure*}[t!]
    ...
    \end{center}
    \caption{Row 1: Activation time-series of the innovation signals estimated by PFM calculated as the sum of squares at every time-frame. Color-bands indicate the onset and duration of each condition in the task (green: tongue, purple: left-hand finger-tapping, blue: right-hand finger-tapping, red: left-foot toes, orange: right-foot toes). Rows 2-6: time-series of a representative voxel for each task with the PFM-estimated innovation (blue) and neuronal-related (i.e., fitted, orange) signals, with their corresponding GLM, PFM and TA maps on the right.}
\label{fig:task_maps}
\end{figure*}
