...
\begin{figure}[t!]
    ...
    \end{center}
    \caption{Activity-inducing co-activation patterns (CAPS, left) and innovation-driven co-activation patterns (iCAPs, right) obtained from PFM-estimated activity-inducing and innovation signals, respectively, using a BIC-based selection of $\lambda$. Time-points selected with a 95th percentile threshold are shown over the average time-series (blue) in the seed region (white-cross) and the deconvolved signal (orange). CAPs and seed correlation maps are illustrated in the center.}
\label{fig:caps}
\end{figure}

\todo[inline]{Should we discuss something about computational time? Or more generally about the algorithmic choices that are possible for PFM and TA.}