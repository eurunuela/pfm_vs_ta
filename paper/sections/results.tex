...
Additionally, in order to describe the extent of the discrepancies between the techniques, we calculated the residual sum of squares (RSS); i.e., the sum of squares of the differences betweeen the estimated activity-inducing or innovation signals of PFM and TA as:
\begin{equation}
    RSS = \frac{\sum{(\mathbf{\hat{s}_{PFM}} - \mathbf{\hat{s}_{TA}})^2}}{N}.
\end{equation}

Figure~\ref{fig:rss} depicts the RSS of the spike and block models for the three experimental datasets introduced: i.e., motor, monoband and multiband. It can be clearly seen that RSS values are lower than those of the activity-inducing and innovation signals, suggesting that the differences betweeen Paradigm Free Mapping and Total Activation are negligible.
