...
\end{equation}
where \(y_t\) is the measured BOLD signal on a given voxel. This equation can be reformulated in matrix notation as \(\mathbf{y} = \mathbf{Hs} + \mathbf{\epsilon}\), where \(\mathbf{y, s} \in \mathbb{R}^N\), \(\mathbf{H} \in \mathbb{R}^{NxN}\) is the HRF in Toeplitz matrix form, \(N\) is the number of frames of the fMRI acquisition, and \(\mathbf{\epsilon}\) represents additional white Guassian noise. The signal model in~\eqref{eq:gitelman} can be extended to represent the neuronal signal \(\mathbf{s}\) in terms of its innovation signal \(\mathbf{u}\), i.e.\ its derivative, and can be described as \(\mathbf{s} = \mathbf{Lu}\) where \(\mathbf{L} \in \mathbb{R}^{N \times N}\) is an integration operator.
...
Even though based on the same signal model as PFM, Total Activation (TA) proposes to use a linear differential operator \(L_h\) that inverts the hemodynamic system based on activelets to recover the activity-inducing signal \(\mathbf{s}\)~\cite{karahanouglu2013total,khalidov2011activelets,karahanoglu2011SignalProcessingApproacha}:
\begin{equation}
    L_h\{x\}(t) = s(t)
\end{equation}
