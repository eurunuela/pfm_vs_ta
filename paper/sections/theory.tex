...
\todo[inline]{I think we need a short paragraph here introducing: 1) our notations; 2) basic math definitions, in particular for norms.}
...
Therefore, for a given voxel, the neuronal-related signal could be estimated by solving the following 
...
[Below needs to be merged together -- It's better to first introduce PFM, then TA, and then unify]

Explain difference between synthesis- and analysis-based formulation here.

backward model formulation relies upon a differential operator to invert the convolution by the HRF

For the purpose of this paper, it will be useful to consider also the model under a slightly different form
$$
