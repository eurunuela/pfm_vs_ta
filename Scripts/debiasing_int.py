...
def read_volumes(paths, n_noise_voxels=10000, discard_pcg=1):
    ...
    if nTE != 1:
        echo_hdr = nib.load(paths['dir'] + paths['datasets'][0]).header
    else:
        echo_hdr = nib.load(paths['dir'] + paths['datasets']).header
...
def export_volumes(volumes, paths, header, nscans, history):
    ...
    if 'beta' in volumes:
        ...
        filename = paths['output_dir'] + '/' + paths['datasets'][1:len(paths['datasets']) - 7] + '.pySPFM.' + lars_key + 'DR2.' + paths['key'] + 'nii.gz'
        nib.save(img, filename)
        print('BETA estimates correctly exported.')
        filename_afni = filename[:-7]
        subprocess.call('3dcopy {} {} -overwrite'.format(filename, filename_afni), shell=True)
        # subprocess.call('3dnewid {}'.format(filename_afni), shell=True)
        subprocess.call('rm {}'.format(filename), shell=True)
        if history is not None:
            print('Updating file history...')
            subprocess.call('3dNotes -h "' + history + '" ' + filename_afni + '+orig.', shell=True)
            print('File history updated.')

    # Fitted signal estimates
    if 'betafitts' in volumes:
        print('Exporting BETAFIITS estimates...')
        nTE = int(np.ceil(volumes['betafitts'].shape[0]/nscans))

        if nTE > 1:
            for teidx in range(nTE):
                img = nib.nifti1.Nifti1Image(volumes['betafitts'][teidx*nscans+1:nscans*(teidx+1),:], None, header=header)
                filename = paths['output_dir'] + '/' + paths['datasets'][1:len(paths['datasets']) - 7] + '.pySPFM.' + lars_key + 'dr2HRF_E0' + str(teidx+1) + '.' + paths['key'] + 'nii.gz'
                nib.save(img, filename)
                subprocess.call('3dnewid {}'.format(filename))
                print('BETAFIITS estimates correctly exported.')
                filename_afni = filename[:-7].copy()
                subprocess.call('3dcopy {} {} -overwrite'.format(filename, filename_afni), shell=True)
                # subprocess.call('3dnewid {}'.format(filename_afni), shell=True)
                subprocess.call('rm {}'.format(filename), shell=True)
                if history is not None:
            ...
            filename = paths['output_dir'] + '/' + paths['datasets'][1:len(paths['datasets']) - 7] + '.pySPFM.' + lars_key + 'dr2HRF.' + paths['key'] + 'nii.gz'
            nib.save(img, filename)
            print('BETAFIITS estimates correctly exported.')
            filename_afni = filename[:-7]
            subprocess.call('3dcopy {} {} -overwrite'.format(filename, filename_afni), shell=True)
    ...
    if 's_zero' in volumes:
        if volumes['s_zero'].size != 0:
            ...
            filename = paths['output_dir'] + '/' + paths['datasets'][1:len(paths['datasets']) - 7] + '.pySPFM.' + lars_key + 's0.' + paths['key'] + 'nii.gz'
            nib.save(img, filename)
    ...
    if 'auc' in volumes:
        if volumes['auc'].size != 0:
            ...
            filename = paths['output_dir'] + '/' + paths['datasets'][1:len(paths['datasets']) - 7] + '.pySPFM.' + lars_key + 'AUC.THR.' + paths['key'] + 'nii.gz'
            nib.save(img, filename)
            print('AUC timecourses correctly exported.')
            filename_afni = filename[:-7]
            subprocess.call('3dcopy {} {} -overwrite'.format(filename, filename_afni), shell=True)
            # subprocess.call('3dnewid {}'.format(filename_afni), shell=True)
            subprocess.call('rm {}'.format(filename), shell=True)
...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='fMRI data deconvolution')
    ...
    parser.add_argument('--dist', type=int, help='Minimum distance between two AUC peaks (default=3).',default=3, nargs=1)
    ...
    if args.auc_dwm is not None:
        ...
        print('No thresholding will be applied on the AUC.')
    ...
    # Reads DWM AUC if input is given
    ...
    # Selects only maximum among consecutive nonzero coefficients
    nonzero_idxs = np.where(auc != 0)[0]
    ...
    for vox_idx in range(nvoxels):
        ...
        # np.savetxt('temp/S_{}.out'.format(vox_idx), np.dot(hrf, S))
    ...
    if args.mask_dwm is not None and args.auc_dwm is not None:
        ...
        output_vols = {'beta': beta_out, 'betafitts': betafitts_out, 'auc': auc_out}
    else:
